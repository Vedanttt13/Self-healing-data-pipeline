...
class DataReader:
    ...
    def data_read(self):
        ...
        try:
            ...
            logger.error(f"Missing required source configuration key: {e}")
            raise
    ...
    def read_jdbc(self):
        ...
        try:
            ...
            logger.info(f"Reading JDBC table: {full_table_name}")
            ...
            logger.error(f"Missing JDBC configuration key: {e}")
            raise

        except Exception as e:
            logger.error(f"Failed to read JDBC source: {str(e)}")
            raise
    ...
    def read_warehouse(self):
        ...
        try:
            ...
            logger.info(f"Reading warehouse table: {table}")

            df = (
                self.spark.read
                .format("parquet")
                .load(f"/warehouse/{table}")
            )
            ...
            logger.error(f"Missing warehouse configuration key: {e}")
            raise

        except Exception as e:
            logger.error(f"Failed to read warehouse source: {str(e)}")
            raise
    ...
    def read_object_storage(self):
        ...
        try:
            ...
            file_format = storage_config.get("format", "parquet")

            logger.info(f"Reading object storage path: {path}")
            ...
            logger.info("Object storage data loaded successfully")
            return df

        except KeyError as e:
            logger.error(f"Missing object storage configuration key: {e}")
            raise

        except Exception as e:
            logger.error(f"Failed to read object storage source: {str(e)}")
            raise